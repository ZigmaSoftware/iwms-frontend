...
// NOTE: unlike gov's original (which colors bins by collected/not_collected
// status), private's shared bin-priority coloring model (BinPriority /
// BIN_PRIORITY_META / getPriorityFromColor in mapUtils.ts and
// BinMapPanel.tsx) is kept as the source of truth and is NOT touched here.
// This file mirrors gov's collected/not_collected data shape 1:1 (it's not
// currently wired into BinMapPanel.tsx — see that file for the actual,
// priority-colored bin map) and uses `createHouseIcon`, private mapUtils'
// status-colored icon factory, instead of `createBinIcon` (which in private
// is BinPriority-typed, not HouseholdStatus-typed).
type Bin = {
  id: string;
  name: string;
  lat: number;
  ...
  useEffect(() => {
    ...
    filteredBins.forEach((bin) => {
      ...
      const marker = L.marker(pos, {
        icon: createHouseIcon(bin.status, isFocused),
        title: bin.name,
      })
        .addTo(map)
        .bindPopup(
