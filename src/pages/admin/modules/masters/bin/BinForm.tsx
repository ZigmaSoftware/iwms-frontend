...
//       wasteTypeApi.list(),
//     ])
//       .then(([wardRes, panchayatRes, collectionPointRes, wasteTypeRes]) => {
...
//               p.unique_id ?? p.panchayat_id ?? p.id ?? p.panchayat
//             ),
//             label: String(
//               p.panchayat_name ?? p.name ?? p.panchayat ?? p.unique_id ?? ""
//             ),
//           }))
...
//           .map((cp) => ({
//             value: normalizeIdValue(
//               cp.unique_id ?? cp.collection_point_id ?? cp.cp_id ?? cp.id
//             ),
//             label: String(
//               cp.cp_name ?? cp.collection_point_name ?? cp.name ?? cp.unique_id ?? ""
//             ),
...
//         setBinCapacity(
//           toNumberOrEmpty(record.bin_capacity ?? record.capacity_liters)
//         );
...
//         const panchayatCandidate = normalizeIdValue(
//           record.panchayat_id ?? record.panchayat
//         );
...
//   useEffect(() => {
//     if (!pendingWard || wards.length === 0) return;

//     if (!wards.some((w) => w.value === pendingWard)) {
//       setWards((prev) => [...prev, { value: pendingWard, label: pendingWard }]);
//     }
...
//       setPanchayats((prev) => [
//         ...prev,
//         { value: pendingPanchayat, label: pendingPanchayat },
//       ]);
//     }
...
//       setWasteTypes((prev) => [
//         ...prev,
//         { value: pendingWasteType, label: pendingWasteType },
//       ]);
//     }
