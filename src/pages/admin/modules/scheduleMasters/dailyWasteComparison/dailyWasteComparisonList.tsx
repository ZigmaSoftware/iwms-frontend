...
import {
  Activity, BarChart3, Calendar, Download, MapPin,
  Pencil, Plus, Scale, TrendingDown, TrendingUp, Trash2, Truck,
} from "lucide-react";
...
type DailyReportRow = {
  ...
  panchayat_id: string; panchayat_name?: string;
  waste_type: string;
...
const initialKpis: DailyReportResponse["kpis"] = {
  total_agreed_weight_kg: 0, total_actual_weight_kg: 0,
  variance_kg: 0, collection_efficiency_percent: 0,
  average_weight_per_trip: 0, coverage_efficiency_percent: 0,
  total_trips: 0, collection_points_covered: 0, report_status: "On Target",
};
...
const fmtKg = (v?: number | string | null, dec = 2) => {
  const n = Number(v);
  return Number.isNaN(n) ? "—" : n.toLocaleString("en-IN", { maximumFractionDigits: dec });
};
...
const effColor = (e: number) =>
  e >= 90
    ? { text: "text-green-600", bg: "bg-green-500", ring: "border-green-200 bg-green-50" }
    : e >= 70
    ? { text: "text-amber-600", bg: "bg-amber-400", ring: "border-amber-200 bg-amber-50" }
    : { text: "text-red-600", bg: "bg-red-500", ring: "border-red-200 bg-red-50" };
...
const PlbEffRow = ({ plb }: { plb: any }) => {
  ...
  return (
    <div className="flex items-center gap-3 py-2.5 border-b border-gray-50 last:border-0">
      ...
      <span className={`text-[10px] font-semibold px-1.5 py-0.5 rounded border ${statusBadgeCls(plb.report_status)}`}>
        {plb.report_status}
      </span>
    </div>
  );
...
export default function DailyWasteComparisonList() {
  ...
  const [plbCompare,   setPlbCompare]   = useState<DailyReportResponse["panchayat_comparison"]>([]);
  ...
  const fetchReport = async () => {
    ...
    if (!companyUniqueId && !isSuperAdmin) {
      setRows([]); setDateTrends([]); setPlbCompare([]); setKpis(initialKpis);
      return;
    }
    ...
    try {
      ...
      setPlbCompare(Array.isArray(data?.panchayat_comparison) ? data.panchayat_comparison : []);
  ...
  useEffect(() => { void fetchReport(); }, [appliedDate, sortMode, companyUniqueId, projectId, companies.length]);
  ...
  const handleDelete = async (row: DailyReportRow) => {
    const result = await Swal.fire({
      ...
      confirmButtonText: t("common.delete"), cancelButtonText: t("common.cancel"),
    });
  ...
  const plbChartData = useMemo(
    () =>
      (plbCompare as any[]).slice(0, 8).map((p) => ({
        name: String(p.panchayat_name ?? p.panchayat_id).replace("Panchayat ", "PLB "),
  ...
  const handleDownload = () => {
    const ws = XLSX.utils.json_to_sheet(
      rows.map((r) => ({
        Date: r.collection_date, PLB: r.panchayat_name ?? r.panchayat_id,
  ...
  return (
    <div className="p-5 space-y-5 bg-gray-50 min-h-screen font-sans">
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-sm px-6 py-5 flex flex-wrap items-center justify-between gap-4">
        <div>
          <h1 className="text-xl font-bold text-gray-800">Daily Waste Collection Comparison</h1>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <input
            type="date" value={dateValue} max={todayValue()}
          ...
          <button
            onClick={() => navigate(`/${encScheduleMasters}/${encDailyWasteComparison}/new`)}
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
        {[
          { label: "Collection Efficiency", value: `${fmtKg(kpis.collection_efficiency_percent)}%`,  accent: "border-t-green-500",  icon: <Activity  className="h-4 w-4" /> },
          { label: "Total Variance",         value: `${fmtKg(kpis.variance_kg)} kg`,                  accent: Number(kpis.variance_kg) >= 0 ? "border-t-emerald-500" : "border-t-red-500", icon: Number(kpis.variance_kg) >= 0 ? <TrendingUp className="h-4 w-4" /> : <TrendingDown className="h-4 w-4" /> },
          { label: "Total Trips",            value: fmtKg(kpis.total_trips, 0),                       accent: "border-t-teal-500",   icon: <Truck     className="h-4 w-4" /> },
          { label: "Points Covered",         value: fmtKg(kpis.collection_points_covered, 0),         accent: "border-t-pink-500",   icon: <MapPin    className="h-4 w-4" /> },
          { label: "Agreed Weight",          value: `${fmtKg(kpis.total_agreed_weight_kg)} kg`,       accent: "border-t-indigo-500", icon: <BarChart3 className="h-4 w-4" /> },
          { label: "Actual Weight",          value: `${fmtKg(kpis.total_actual_weight_kg)} kg`,       accent: "border-t-cyan-500",   icon: <Scale     className="h-4 w-4" /> },
        ].map((k) => (
          <div key={k.label} className={`bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden border-t-4 ${k.accent} flex flex-col gap-2 p-4`}>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
          <h2 className="text-sm font-semibold text-gray-800">Date-wise Collection Trend</h2>
          <p className="text-xs text-gray-400 mt-0.5 mb-4">Agreed vs Actual collection weight per date</p>
          {dateTrends.length === 0 ? (
            <div className="h-52 flex items-center justify-center text-gray-400 text-sm">No trend data yet.</div>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <AreaChart data={dateTrends} margin={{ top: 6, right: 20, left: 0, bottom: 4 }}>
                <defs>
                ...
                <XAxis dataKey="collection_date" tick={{ fontSize: 11, fill: "#9ca3af" }} axisLine={false} tickLine={false} />
                ...
                <Area type="monotone" dataKey="agreed_weight_kg" name="Agreed" stroke="#2563eb" strokeWidth={2} strokeDasharray="5 3" fill="url(#gradAgreedD)" dot={{ r: 4, fill: "#2563eb", stroke: "#fff", strokeWidth: 2 }} activeDot={{ r: 6 }} />
                <Area type="monotone" dataKey="actual_weight_kg" name="Actual" stroke="#16a34a" strokeWidth={2.5} fill="url(#gradActualD)" dot={{ r: 4, fill: "#16a34a", stroke: "#fff", strokeWidth: 2 }} activeDot={{ r: 6 }} />
              </AreaChart>
            </ResponsiveContainer>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5">
          <h2 className="text-sm font-semibold text-gray-800">PLB Collection Efficiency</h2>
          <p className="text-xs text-gray-400 mt-0.5 mb-4">Actual ÷ Agreed — per PLB (Participating Local Body)</p>
          {plbCompare.length === 0 ? (
            <div className="h-52 flex items-center justify-center text-gray-400 text-sm">No PLB data yet.</div>
          ) : (
            <div className="space-y-1 max-h-[220px] overflow-y-auto pr-1">
              {(plbCompare as any[]).slice(0, 10).map((p, i) => <PlbEffRow key={i} plb={p} />)}
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 lg:col-span-2">
          <h2 className="text-sm font-semibold text-gray-800">PLB Collection — Agreed vs Actual</h2>
          <p className="text-xs text-gray-400 mt-0.5 mb-4">
            Side-by-side daily totals per PLB&nbsp;·&nbsp;
            <span className="inline-block w-2.5 h-2.5 rounded-sm bg-blue-300 mr-1 align-middle" />Agreed&nbsp;&nbsp;
            <span className="inline-block w-2.5 h-2.5 rounded-sm bg-green-600 mr-1 align-middle" />Actual
          </p>
          {plbChartData.length === 0 ? (
            <div className="h-52 flex items-center justify-center text-gray-400 text-sm">No data.</div>
          ) : (
            <ResponsiveContainer width="100%" height={220}>
              <BarChart data={plbChartData} margin={{ top: 6, right: 16, left: 0, bottom: 56 }} barCategoryGap="30%" barGap={4}>
                <CartesianGrid strokeDasharray="3 3" stroke="#f3f4f6" vertical={false} />
                <XAxis dataKey="name" tick={{ fontSize: 10, fill: "#9ca3af", angle: -35, textAnchor: "end" }} axisLine={false} tickLine={false} interval={0} />
                <YAxis tick={{ fontSize: 11, fill: "#9ca3af" }} axisLine={false} tickLine={false} tickFormatter={fmtAxis} />
                <Tooltip content={<PLBTooltip />} />
                <Legend iconType="square" iconSize={10} wrapperStyle={{ fontSize: 11, paddingTop: 14 }} />
                <Bar dataKey="Agreed" fill="#93c5fd" maxBarSize={36} radius={[3, 3, 0, 0]}>
                  {plbChartData.map((_, i) => <Cell key={i} fill="#93c5fd" />)}
                </Bar>
                <Bar dataKey="Actual" fill="#16a34a" maxBarSize={36} radius={[3, 3, 0, 0]}>
                  {plbChartData.map((e, i) => (
                    <Cell key={i} fill={e.Actual >= e.Agreed ? "#16a34a" : "#f97316"} />
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
        ...
        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-base font-bold text-gray-800">Daily Collection Summary</p>
            ...
            <span className={`inline-flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-bold border ${statusBadgeCls(reportStatus)}`}>
              {reportStatus === "Deficit"   && <TrendingDown className="h-3 w-3" />}
          ...
          <div className="p-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            <StatCell label="Total Agreed Weight"    value={`${fmtKg(kpis.total_agreed_weight_kg)} kg`}        color="border-blue-100 bg-blue-50" />
            <StatCell label="Total Actual Weight"    value={`${fmtKg(kpis.total_actual_weight_kg)} kg`}        color="border-green-100 bg-green-50" />
            <StatCell label="Total Variance"         value={`${fmtKg(kpis.variance_kg)} kg`}                   color={Number(kpis.variance_kg) >= 0 ? "border-emerald-100 bg-emerald-50" : "border-red-100 bg-red-50"} />
            <StatCell label="Collection Efficiency"  value={`${fmtKg(kpis.collection_efficiency_percent)}%`}   color={`${effC.ring} border`} />
          </div>
            ...
            <div className="h-4 rounded-full bg-gray-100 overflow-hidden relative">
              ...
              {eff >= 10 && (
                <span className="absolute inset-0 flex items-center justify-center text-[10px] font-bold text-white">
                  {fmtKg(kpis.total_actual_weight_kg)} / {fmtKg(kpis.total_agreed_weight_kg)} kg
                </span>
              )}
            </div>
          </div>
          ...
          <div className="border-t border-gray-100 px-6 py-4 grid grid-cols-2 md:grid-cols-4 gap-4 bg-gray-50/60">
            {[
              { label: "Total Trips",        value: fmtKg(kpis.total_trips, 0),                        icon: <Truck    className="h-4 w-4 text-teal-500"   /> },
              { label: "Points Covered",      value: fmtKg(kpis.collection_points_covered, 0),          icon: <MapPin   className="h-4 w-4 text-pink-500"   /> },
              { label: "Coverage Efficiency", value: `${fmtKg(kpis.coverage_efficiency_percent ?? 0)}%`, icon: <Activity className="h-4 w-4 text-purple-500" /> },
              { label: "Avg Weight / Trip",   value: `${fmtKg(kpis.average_weight_per_trip ?? 0)} kg`,  icon: <Scale    className="h-4 w-4 text-indigo-500" /> },
            ].map((s) => (
            ...
            <div className="border-t border-gray-100 px-6 py-5">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
                {(plbCompare as any[]).slice(0, 8).map((p, i) => {
                  ...
                  const pAgreed = Number(p.agreed_weight_kg ?? p.total_agreed_weight ?? 0);
                  const pActual = Number(p.actual_weight_kg ?? p.total_actual_weight ?? 0);
                  return (
                    <div key={i} className="bg-white rounded-xl border border-gray-200 p-3.5 hover:shadow-md transition-shadow">
                      <div className="flex items-start justify-between mb-2">
                        <div>
                          <p className="text-xs font-bold text-gray-800">{p.panchayat_name ?? p.panchayat_id}</p>
                          <p className="text-[10px] text-gray-400 mt-0.5">
                            {p.waste_type ?? "All types"}
                          </p>
                        </div>
                        <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded border ${statusBadgeCls(p.report_status)}`}>
                          {p.report_status}
                        </span>
                      </div>
                      <div className="mb-2">
                        <div className="flex justify-between text-[10px] text-gray-400 mb-1">
                          ...
                          <span className={`font-bold ${pc.text}`}>{pEff.toFixed(1)}%</span>
                        </div>
                        <div className="h-1.5 rounded-full bg-gray-100 overflow-hidden">
                          <div className={`h-full rounded-full ${pc.bg}`} style={{ width: `${Math.min(pEff, 100)}%` }} />
                        </div>
                      </div>
                      <div className="grid grid-cols-2 gap-1.5">
                        <div className="text-center bg-blue-50 rounded-lg py-1.5">
                          <p className="text-[10px] text-blue-500 font-medium">Agreed</p>
                          <p className="text-xs font-bold text-blue-700">{fmtKg(pAgreed)} kg</p>
                        </div>
                        <div className="text-center bg-green-50 rounded-lg py-1.5">
                          <p className="text-[10px] text-green-500 font-medium">Actual</p>
                          <p className="text-xs font-bold text-green-700">{fmtKg(pActual)} kg</p>
                        </div>
                      </div>
                      {rows.filter((r) => r.panchayat_id === p.panchayat_id).length > 0 && (
                        <div className="mt-2 flex justify-between items-center">
                          <span className="text-[10px] text-gray-400">
                            Trips: <strong className="text-gray-600">
                              {rows.filter((r) => r.panchayat_id === p.panchayat_id).reduce((s, r) => s + r.total_trips, 0)}
                            </strong>
                          </span>
                          <button
                            onClick={() =>
                              navigate(`/${encScheduleMasters}/${encDailyWasteComparison}/${rows.find((r) => r.panchayat_id === p.panchayat_id)?.unique_id}/edit`, {
                                state: { record: rows.find((r) => r.panchayat_id === p.panchayat_id) },
                              })
                            }
