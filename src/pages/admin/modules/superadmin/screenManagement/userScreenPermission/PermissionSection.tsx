...
export default function PermissionSection({
  ...
  ----------------------------------------------------------- */
  useEffect(() => {
    if (!hasScope || !mainScreenId) return;
    ...
    adminApi.companyWiseScreenPermissions
      .read(`by-project-format/?${query.toString()}`)
      .then((res: any) => {
        if (cancelled) return;
  ...
  }, [hasScope, companyId, projectId, mainScreenId, permissionType]);
  ...
  /* -----------------------------------------------------------
     FETCH COLUMNS FOR EACH SCREEN IN MATRIX (Field Permission only)
  ----------------------------------------------------------- */
  ...
  }, [screenIdsKey, hasScope, projectId, companyId, permissionType]);
