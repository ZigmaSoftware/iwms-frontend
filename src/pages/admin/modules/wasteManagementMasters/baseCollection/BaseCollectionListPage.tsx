...
export default function BaseCollectionListPage({ scope }: Props) {
  ...
  const [dialogFilters, setDialogFilters] =
    useState<DialogFilters>(EMPTY_DIALOG_FILTERS);
  ...
  const fetchRows = useCallback(async () => {
    try {
      ...
      rows.forEach((row) => {
        const id = scope === "panchayat" ? row.panchayat_id : row.ward_id;
        const name =
          scope === "panchayat" ? row.panchayat_name : row.ward_name;
        const weight =
          scope === "panchayat"
            ? row.panchayat_total_weight
            : row.ward_total_weight;
        ...
        if (!grouped[key]) {
          grouped[key] = {
            ...
            records: [],
            zone_name:
              scope === "ward" ? row.zone_name ?? "-" : undefined,
          };
        }
    ...
    return summaryRows.filter((row) => {
      ...
      if (searchText) {
        ...
        if (
          ...
          !(row.zone_name ?? "").toLowerCase().includes(q)
        ) {
          return false;
        }
      }
      ...
      if (columnFilters.zone_name) {
        if (
          !(row.zone_name ?? "")
            .toLowerCase()
            .includes(columnFilters.zone_name.toLowerCase())
        ) {
          return false;
        }
      }
  ...
  }, [summaryRows, searchText, zoneFilter, columnFilters]);
  ...
  const indexTemplate = (_: unknown, { rowIndex }: any) => rowIndex + 1;
  ...
  return (
    <div className="p-4">
      ...
      <div className="mb-4 flex gap-3 text-sm">
        <span className="bg-slate-100 px-4 py-2 rounded-full">
          Daily: {filteredDailyWeight}
        </span>
        <span className="bg-slate-100 px-4 py-2 rounded-full">
          Overall: {filteredOverallWeight}
        </span>
      </div>

      <DataTable value={filteredSummaryRows} paginator rows={10} stripedRows showGridlines>
        ...
        <Column header="Records" body={(row) => row.count} />
        <Column
          header="Total Weight"
          body={(row) => row.total_weight.toFixed(2)}
        />
      </DataTable>
    </div>
  );
