...
/**
 * Mirrors the original inline check in TripAttendanceForm: on create,
 * daily_trip_assignment_id/staff_id/vehicle_id are required (they're locked
 * once a trip is in progress, so edit mode never re-validates them);
 * latitude/longitude/source are required whenever visible, in both create
 * and edit mode. Takes `isEdit` and `showField` as parameters because the
 * requirement is conditional on both, which the simpler `requireWhenVisible`
    ...
    .superRefine((data, ctx) => {
      if (!isEdit && showField("daily_trip_assignment_id") && !data.daily_trip_assignment_id) {
        ctx.addIssue({
          ...
          path: ["daily_trip_assignment_id"],
          message: "Daily trip assignment is required",
        });
      }
